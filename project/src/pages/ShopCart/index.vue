...
import {mapGetters} from 'vuex'
  export default {
    ...
    methods:{
      // 获取购物车个人数据
      getData(){
      // let result =   localStorage.getItem('UUIDTOKEN')
      //   console.log(result);
 ...
 // type:为了区分这三个元素
        ...
        switch(type){
          ...
          case "add":
            ...
            break;
          case "minus":
            // 判断产品的个数大于一，才可以传递给服务器
            // 如果出现产品的个数小于等于一，传递给服务器个数0【原封不动】
            disNum = cart.skuNum > 1 ? -1 : 0;
            break;
          case "change":
            // 用户输入进来的最终量，如果是非法的（带有汉字|出现负数），带给服务器为零
     ...
     async deleteCartById(cart){
      ...
      // 修改购物车中某一个商品的checked状态
      updateChecked(cart,event){
        let isChecked = event.target.checked ? '1':'0';
        try {
          // 如果成功，再次获取服务器数据
          this.$store.dispatch('uodateCheckedById',{skuId:cart.skuId,isChecked})
          this.getData();
        } catch (error) {
          alert(error.message)
        }
